...
public sealed class WorkspaceController(IWarehouseAnalytics analytics) : Controller
{
    ...
    {
        var m = ModelFor("Clarifications", "Clarification Needed", "Requisitions that need a closer look before approval.");
    ...
    public async Task<IActionResult> Preview(string reportId, CancellationToken token)
    {
        ...
        var m = ModelFor("Preview", title, "Review the report scope before printing."); m.Report = reportId; m.Applied = true;
    ...
    private static DataTable DeadStockTable(IEnumerable<ReorderAnalysis> rows) => new("Dead stock · no issue in the last 180 days", ["Item ID", "Quantity", "Last issue", "Recommendation"], rows.Select(x => new[] { x.ItemId, N(x.StockOnHand), x.LastIssueDate?.ToString("dd MMM yyyy", C) ?? "No issue", "Review for disposal" }).ToArray());
    private static DataTable SupplierTable(IEnumerable<SupplierPerformance> rows) => new("Supplier delivery reliability", ["Supplier / ID", "Delivered", "On-time", "On-time ratio", "Late", "Average late delay", "Standing"], rows.Select(x => new[] { $"{x.SupplierName} · {x.SupplierId}", x.DeliveredOrders.ToString(), x.OnTimeOrders.ToString(), x.OnTimeRatio.ToString("P0", C), x.LateOrders.ToString(), x.AverageLateDelayDays is null ? "No late deliveries" : $"{N(x.AverageLateDelayDays.Value)} days", Standing(x.Standing) }).ToArray());
    ...
    private static DataTable CandidateTable(RequisitionMatch x) => new("Catalogue candidates", ["Rank", "Item", "Similarity", "Common terms"], x.Candidates.Select(c => new[] { c.Rank.ToString(), c.ItemId, c.Similarity.ToString("P0", C), c.CommonTerms.ToString() }).ToArray());
